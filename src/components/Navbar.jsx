...
const Navbar = () => {
  ...
  const handleSelect = (data) => {
    debugger;
    let id = Number(productTitle.find((item) => item.title === data).id);
    // setResults(search);
    navigate(`/product/${id}`);
  };
  ...
  const searchProduct = async (q) => {
    ...
    if (data) {
      data.products.forEach((element) => {
        productTitle.push({ id: element.id, title: element.title });
      });
      let titles = [];
      productTitle.forEach((item) => {
        console.log(item, results, renderItem(item.title));
        titles.push(renderItem(item.title));
      });
      setResults(titles);
    }
  };
        ...
        <div className="container d-flex py-5">
          <Link to="/">
            ...
            <a href="/#">
              ...
              >
                <g
                  id="b90422e4-2e93-436a-80f6-2b99d9cb717b"
                  data-name="Layer 2"
                >
                  <g
          ...
          >
            <Input.Search
              type="search"
              placeholder="Search (e.g: jeans, iphone)"
            />
          </AutoComplete>
          ...
          <div className="d-flex menu-item">
            ...
            <Link to="/cart">
              <div className="d-flex">
                ...
                />
                {cartQuantity > 0 && (
                  <span style={{ color: "#fff" }}>{cartQuantity}</span>
                )}
              </div>
            </Link>
          </div>
